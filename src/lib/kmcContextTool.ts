...
export const KMC_CONTEXT: KMCContext = {
  departments: {
    ...
    waterSupply: {
      ...
      portalLink: "https://web.kolhapurcorporation.gov.in/citizen",
      departmentLink:
        "https://web.kolhapurcorporation.gov.in/department?deptid=6",
      stepByStepProcess: {
    ...
    commonServices: [
      {
        ...
        process:
          "Login → Navigate to Service #8 → Register new complaint → Track status",
      },
      {
        service: "RTI Application",
        link: "Service #12: 'आरटीआय पोर्टल्स' (RTI Portals)",
        process:
          "Login → Navigate to Service #12 → Fill RTI application → Submit with fees",
      },
      {
        service: "Waste Collection Schedule",
        link: "Service #9: 'घन कचरा संकलन वेळापत्रक'",
        process:
          "Login → Navigate to Service #9 → Select your ward → View schedule",
      },
    ],
  },
  ...
  paymentInfo: {
    ...
    securePayment:
      "KMC uses secure, government-approved payment gateways integrated within the citizen portal for all transactions.",
  },
};
...
export const kmcContextTool = tool({
  description:
    "Access specific KMC (Kolhapur Municipal Corporation) information, step-by-step processes, form filling instructions, official portal navigation, and disaster management information",
  inputSchema: z.object({
    category: z
      .enum(["departments", "generalProcess", "paymentInfo"])
      .describe("Category of KMC information to retrieve"),
    subcategory: z
      .string()
      .optional()
      .describe(
        "Specific subcategory within the main category (e.g., 'disasterManagement', 'propertyTax', 'waterSupply', 'citizenRegistration')",
      ),
  }),
  execute: async ({ category, subcategory }): Promise<ToolResult> => {
    ...
    if (
      ...
      if (
        ...
        subcategory in (typedCategoryData as Departments)
      ) {
        return {
          category: category as ValidCategory,
          subcategory,
          data: (typedCategoryData as Departments)[
            subcategory as keyof Departments
          ],
        };
      }
      if (
        category === "generalProcess" &&
        subcategory in (typedCategoryData as GeneralProcess)
      ) {
        return {
          category: category as ValidCategory,
          subcategory,
          data: (typedCategoryData as GeneralProcess)[
            subcategory as keyof GeneralProcess
          ],
        };
      }
      if (
        category === "paymentInfo" &&
        subcategory in (typedCategoryData as PaymentInfo)
      ) {
        return {
          category: category as ValidCategory,
          subcategory,
          data: (typedCategoryData as PaymentInfo)[
            subcategory as keyof PaymentInfo
          ],
        };
      }
    }
